  ...
  try {
    ...
    if (filter === "yearly") {
      ...
      yearlyRevenues.forEach((yearData, index) => {
        graphValue[index] =
          yearData.yearlyRevenue.length > 0
            ? yearData.yearlyRevenue[0].yearlyRevenue
            : 0;
      });
    } else {
    ...
    const topProductLabels = topSellingProducts.map(
      (product) => product.productName
    );
    const topProductCounts = topSellingProducts.map(
    ...
    const topCategoryLabels = topSellingCategories.map(
      (category) => category.categoryName
    );
    const topCategoryCounts = topSellingCategories.map(
